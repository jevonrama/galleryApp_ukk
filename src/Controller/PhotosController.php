...
class PhotosController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            $namefile = $this->request->getData()['image']->getClientFileName();
            $extfile = substr(strchr($namefile, '.'), 1);


            $pathfile = "upload/".$namefile.".".$extfile;
            $photo = $this->Photos->patchEntity($photo, $this->request->getData());
            $photo->image = $namefile.".".$extfile;

            $files['image']->moveTo(WWW_ROOT.$pathfile);
    ...
    public function edit($id = null)
    {
        ...
        // Memuat data pengguna dan album untuk ditampilkan dalam formulir
        $users = $this->Photos->Users->find('list')->limit(200)->toArray();
        $albums = $this->Photos->Albums->find('list')->limit(200)->toArray();
        $this->set(compact('photo', 'users', 'albums'));
    }
